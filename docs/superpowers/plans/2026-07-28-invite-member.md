...
- Files under `src/app/**` and `tests/**` use double-quoted strings (matches `src/app/settings/actions.ts`, `src/app/login/page.tsx`). Files under `src/application/**`, `src/features/**`, and `src/lib/**` use single-quoted strings (matches `src/application/update-organization.ts`, `src/lib/supabase/env.ts`).
