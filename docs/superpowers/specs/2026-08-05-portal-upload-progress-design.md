...
> **Deviation, recorded (final branch review, New Minor #1):** the automatic-retry-on-`upload_finalize_in_progress` behavior described above was intentionally dropped at the implementation-plan level — the shipped UI surfaces the message and asks for a manual retry instead. Accepted as low-impact (the single-tab UI calls finalize exactly once per session), not a bug.
