...
A Case Access grant SHALL reference one Client, one Case, and the Case's Organization. A grant SHALL confer access to that Case alone and SHALL NOT extend to sibling Cases of the same Client.

#### Scenario: Grant does not extend to a second case
- **GIVEN** Client C holds an active grant to Case 1 and no grant to Case 2 in the same Organization
